...
router.get('/', [
  ...
  try {
    ...
    const orderByClause = `ORDER BY i.${sortBy} ${(sortOrder as string).toUpperCase()}`;
...
router.post('/bulk-update', [
  ...
  try {
    ...
    return res.json({ message: 'Bulk update completed successfully', results });
...
router.get('/search/upc/:upc', [
  ...
  try {
    ...
    return res.status(500).json({ error: 'Failed to search by UPC' });
  }
});
...
router.get('/alerts/overstocked', async (req: Request, res: Response) => {
  try {
    ...
    return res.status(500).json({ error: 'Failed to fetch overstocked items' });
  }
});
